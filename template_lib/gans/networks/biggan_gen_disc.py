...
def G_arch(ch=64, attention='64', ksize='333333', dilation='111111'):
  arch = {}
  arch[1024] = {'in_channels' : [ch * item for item in [16, 16, 16, 8, 8, 4, 2, 1]],
                'out_channels': [ch * item for item in [16, 16, 8, 8, 4, 2, 1, 1]],
                'upsample'    : [True] * 8,
                'resolution'  : [8, 16, 32, 64, 128, 256, 512, 1024],
                'attention'   : {2 ** i: (
                      2 ** i in [int(item) for item in attention.split('_')])
                  for i in range(3, 11)}}
  ...
  arch[32] = {'in_channels' : [ch * item for item in [4, 4, 4]],
              ...
              'attention'   : {
                ...
                for i in range(3, 6)}}
  arch[48] = {'in_channels' : [ch * item for item in [4, 4, 4]],
              'out_channels': [ch * item for item in [4, 4, 4]],
              'upsample'    : [True] * 3,
              'resolution'  : [8, 16, 32],
              'attention'   : {2 ** i: (2 ** i in [int(item) for item in attention.split('_')])
                               for i in range(3, 6)}}
  arch['test'] = {'in_channels' : [ch * item for item in [16, 8, 4]],
                  'out_channels': [ch * item for item in [8, 4, 4]],
                  'upsample'    : [True] * 3,
                  'resolution'  : [8, 16, 32],
                  'attention'   : {
                    2 ** i: (2 ** i in [int(item) for item in attention.split('_')])
...
def D_arch(ch=64, attention='64', ksize='333333', dilation='111111'):
  arch = {}
  arch[1024] = {'in_channels' : [3] + [ch * item for item in [1, 1, 2, 4, 8, 8, 16, 16]],
                'out_channels': [item * ch for item in [1, 1, 2, 4, 8, 8, 16, 16, 16]],
...
class BigGANDisc(nn.Module):
  ...
  def __init__(self, cfg, **kwargs):
    ...
    self.ch                       = get_attr_kwargs(cfg, 'ch', default=64, **kwargs)
    ...
    self.activation               = get_attr_kwargs(cfg, 'activation', default=nn.ReLU(), **kwargs)
    self.SN_eps                   = get_attr_kwargs(cfg, 'SN_eps', default=1e-6, **kwargs)
    self.output_dim               = get_attr_kwargs(cfg, 'output_dim', default=1, **kwargs)
    self.init                     = get_attr_kwargs(cfg, 'init', default='ortho', **kwargs)
    self.D_param                  = get_attr_kwargs(cfg, 'D_param', default='SN', **kwargs)
    self.use_cdisc                = get_attr_kwargs(cfg, 'use_cdisc', default=True, **kwargs)

    # Architecture
    self.arch = D_arch(self.ch, self.attention)[self.img_size]
