...
class FID_IS_Score_Pytorch(object):
  def __init__(self, parallel=True, device=0):
    # Load network
    net = load_inception_net(parallel)
    self.net = net.cuda(device)

  def calculate_fid_stat_for_pytorch_dataset(
          self, loaders, saved_inception_moments, stdout=sys.stdout):
    """
    ...
    import tqdm
    pool, logits, labels = [], [], []
    pbar = tqdm.tqdm(loaders, file=stdout,
                     desc='calculate_fid_stat_for_pytorch_dataset')
    ...
    print('IS: %5.5f +/- %5.5f'%(IS_mean, IS_std))
    ...
    print('Save pytorch fid_stat in %s'%saved_inception_moments)
    pass
