...
class HingeLossCondDynamicNCritic(object):
  ...
  def __init__(self, cfg, **kwargs):
    ...
    self.d_loss_margin          = get_attr_kwargs(cfg, 'd_loss_margin', default=0.4, **kwargs)
  ...
  def __call__(self, images, labels, z, iteration, ema=None, **kwargs):
    ...
    if d_loss.item() < self.d_loss_margin:
      self.G.zero_grad()
      z_sample = z.sample()
      z_sample = z_sample.to(self.device)
      gy = dy
...
@GAN_MODEL_REGISTRY.register()
class HingeLossCondFocal(object):
  ...
  def __call__(self, images, labels, z, iteration, ema=None, **kwargs):
    ...
    if iteration % self.n_critic == 0:
      ...
      G_f_logit_mean, g_loss = self.hinge_loss_generator(r_logit=d_real, f_logit=d_fake_g, b=self.b)
      summary_d['d_logit_mean']['G_f_logit_mean'] = G_f_logit_mean.item()
      summary_d['g_loss']['g_loss'] = g_loss.item()
  ...
  @staticmethod
  def hinge_loss_generator(r_logit, f_logit, b=1.):
    f_logit_mean = f_logit.mean()

    alpha = F.relu(b - r_logit.detach()) + F.relu(b + f_logit.detach())
    beta = torch.relu(-1. / (2*b) * alpha + 1).detach()

    num_posi = (beta > 0).sum().item()
    if num_posi == 0 :
      num_posi = 1
    G_loss = - (beta * f_logit).sum() / num_posi
    return f_logit_mean, G_loss
