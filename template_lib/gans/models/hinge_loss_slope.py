...
class SlopeHingeLossCond(object):
  ...
  def __call__(self, images, labels, z, iteration, ema=None, **kwargs):
    ...
    if iteration % self.log_every == 0:
      test_z_sample = z.sample().to(self.device)
      test_gy = dy
      test_fake = self.G(test_z_sample, y=test_gy, **kwargs)
      test_d_fake_g = self.D(test_fake, y=test_gy, **kwargs)
      g = torch.autograd.grad(test_d_fake_g, test_fake,
                              grad_outputs=torch.ones(test_d_fake_g.size()).cuda(),
                              create_graph=False)[0].view(test_d_fake_g.size(0), -1)
      gp = g.norm(p=2, dim=1)
