...
class HybridTrainPipe_CIFAR(Pipeline):
  def __init__(self, batch_size, num_threads, device_id, data_dir, crop=32,
               dali_cpu=False, local_rank=0,
               world_size=1,
               cutout=0):
...
class CIFAR_INPUT_ITER():
  ...
  def __init__(self, batch_size, type='train',
               root='/userhome/memory_data/cifar10'):
    self.root = root
...
def get_cifar_iter_dali(type, image_dir, batch_size, num_threads, local_rank=0,
                        world_size=1, val_size=32, cutout=0):
  ...
  if type == 'train':
    ...
    pip_val = HybridValPipe_CIFAR(batch_size=batch_size,
                                  num_threads=num_threads, device_id=local_rank,
                                  data_dir=image_dir,
                                  crop=32, size=val_size, world_size=world_size,
...
def get_cifar_iter_torch(type, image_dir, batch_size, num_threads, cutout=0):
  ...
  if type == 'train':
    ...
    train_dst = CIFAR10(root=image_dir, train=True, download=True,
                        transform=transform_train)
    train_iter = torch.utils.data.DataLoader(train_dst, batch_size=batch_size,
    ...
    test_dst = CIFAR10(root=image_dir, train=False, download=True,
                       transform=transform_test)
    test_iter = torch.utils.data.DataLoader(test_dst, batch_size=batch_size,
...
if __name__ == '__main__':
  """
  from dali_cifar10 import get_cifar_iter_dali
  train_loader = get_cifar_iter_dali(type='train',
                                     image_dir='~/.keras/cifar10',                                            
