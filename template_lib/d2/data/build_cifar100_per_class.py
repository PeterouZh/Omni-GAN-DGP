...
def get_dict(registed_name, class_name, data_path, subset, **kwargs):
  ...
  for (img, label) in data_iter:
    if label != c100_dataset.class_to_idx[class_name]:
      continue
    record = {}
    record["image_id"] = idx
    idx += 1
...
def find_classes(data_path):
  c100_dataset = datasets.CIFAR100(root=data_path, train=True, download=True)
  classes = c100_dataset.classes
  class_to_idx = c100_dataset.class_to_idx
...
for name, registed_func, kwargs in zip(registed_name_list, registed_func_list, kwargs_list):
  ...
  for class_name, idx in tqdm.tqdm(class_to_idx.items(), desc=f"Registering cifar100 per class: {name}"):
