...
class Generator(nn.Module):
  ...
  def inference(self, z, y, shape=None, max_points_every=90000):
    if shape is None:
      shape = [self.resolution, ] * 2
      # If hierarchical, concatenate zs and ys
    if self.hier:
    ...
    coord = coord.unsqueeze(0).expand(h.shape[0], *coord.shape[-2:])
    # cell = make_cell(shape).cuda()
    cell = make_cell([self.resolution, self.resolution]).cuda()
    cell = cell.unsqueeze(0).expand(h.shape[0], *cell.shape[-2:]).clone()

    feat = F.unfold(h, 3, padding=1).view(h.shape[0], h.shape[1] * 9, h.shape[2], h.shape[3])

    cell[:, :, 0] *= feat.shape[-2]
    cell[:, :, 1] *= feat.shape[-1]
    if cell.size(1) != coord.size(1):
      cell = torch.ones_like(coord) * 2
    ...
    for idx in range(num_iters):
      ...
      part_cell = cell[:, idx * max_points_every:(idx + 1) * max_points_every]

      part_pred = self._sample_rgb(feat=feat, coord=part_coord, cell=part_cell)
  ...
  def _sample_rgb(self, feat, coord, cell, ):
    sampled_feat = F.grid_sample(
      feat, coord.flip(-1).unsqueeze(1), mode='bilinear', align_corners=False)[:, :, 0, :].permute(0, 2, 1)
...
class Discriminator(nn.Module):
  ...
  def __init__(self, cfg, D_activation=nn.ReLU(inplace=True), verbose=True, **kwargs):
    super(Discriminator, self).__init__()
    kwargs['tl_ret_kwargs'] = {}
    # fmt: off
    # Width multiplier
    self.ch                            = get_attr_kwargs(cfg, 'D_ch', default=64, **kwargs)
    ...
    output_dim                         = get_attr_kwargs(cfg, 'output_dim', default=1, **kwargs)
    optim_cfg                          = get_attr_kwargs(cfg, 'optim_cfg', default={}, **kwargs)
...
class G_D(nn.Module):
  ...
  def forward(self, z, gy, x=None, dy=None, train_G=False, return_G_z=False,
              split_D=False, shape=None):
    # If training G, enable grad tape
    with torch.set_grad_enabled(train_G):
      # Get Generator output given noise
      G_z = self.G(z, self.G.shared(gy), shape=shape)
      # Cast as necessary
