...
from exp.omniGAN.omni_loss import OmniLoss
...
def GAN_training_function(G, D, GD, z_, y_, ema, state_dict, config):
  ...
  omni_loss_cfg = global_cfg.omni_loss
  # fmt: off
  margin              = omni_loss_cfg.margin
  gamma               = omni_loss_cfg.gamma
  # fmt: on
  ...
  def train(x, y):
    ...
    for accumulation_index in range(config['num_G_accumulations']):    
      z_.sample_()
      y_.sample_()
      D_fake = GD(z_, y_, train_G=True, split_D=config['split_D'], shape=img_shape)
...
max2keep_sample_images = MaxToKeep(max_to_keep=5)
...
def save_and_sample(G, D, G_ema, z_, y_, fixed_z, fixed_y, 
  ...
  image_filename = '%s/%s/fixed_samples_%010d.jpg' % (config['samples_root'],
                                                  experiment_name,
                                                  state_dict['itr'])
  torchvision.utils.save_image(fixed_Gz.float().cpu(), image_filename,
                             nrow=int(fixed_Gz.shape[0] **0.5), normalize=True)
  max2keep_sample_images.step(file_path=image_filename)
