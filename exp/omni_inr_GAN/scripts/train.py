...
def run(config):
  ...
  if 'Generator' in global_cfg and 'register_modules' in global_cfg.Generator:
    G = build_model(global_cfg.Generator).to(device)
  else:
    G = model.Generator(**config).to(device)
  if 'Discriminator' in global_cfg and 'register_modules' in global_cfg.Discriminator:
    D = build_model(global_cfg.Discriminator).to(device)
  else:
  ...
  if config['ema']:
    ...
    if 'Generator' in global_cfg and 'register_modules' in global_cfg.Generator:
      G_ema = build_model(global_cfg.Generator, kwargs_priority=True,
  ...
  elif config['which_train_fn'] in ['exp.omniGAN.train_fns', 'exp.omni_inr_GAN.train_fns']:
  ...
  if 'evaluation' in global_cfg and global_cfg.evaluation.eval:
    ...
    G_ema_ckpt = VisualModelCkpt(model=G_ema)
    G_ema_ckpt.load_from_path(cfg.G_ema_model)
    del G_ema_ckpt
